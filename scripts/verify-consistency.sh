...
echo "📋 [12/13] Checking security imports in api-key-handler.ts..."
...
# Check 13: maxLength consistency between OAuth and API key paths
echo "📋 [13/13] Checking maxLength consistency between auth paths..."
if [ ! -f "src/server.ts" ] || [ ! -f "src/api-key-handler.ts" ]; then
  echo "⚠️  WARNING: Cannot check maxLength (missing source files)"
  WARNINGS=$((WARNINGS + 1))
else
  # Extract all maxLength values from both files
  OAUTH_LENGTHS=$(grep -oE "maxLength:\s*[0-9]+" src/server.ts 2>/dev/null | grep -oE "[0-9]+" | sort -u)
  API_LENGTHS=$(grep -oE "maxLength:\s*[0-9]+" src/api-key-handler.ts 2>/dev/null | grep -oE "[0-9]+" | sort -u)

  if [ -z "$OAUTH_LENGTHS" ] && [ -z "$API_LENGTHS" ]; then
    echo "✅ No maxLength specified (unlimited output)"
  elif [ -z "$OAUTH_LENGTHS" ] || [ -z "$API_LENGTHS" ]; then
    echo "⚠️  WARNING: maxLength defined in one auth path but not the other"
    echo "   OAuth path (server.ts): ${OAUTH_LENGTHS:-undefined}"
    echo "   API key path (api-key-handler.ts): ${API_LENGTHS:-undefined}"
    echo "   Both paths should use identical security settings"
    WARNINGS=$((WARNINGS + 1))
  else
    # Convert to arrays for comparison
    OAUTH_ARRAY=($OAUTH_LENGTHS)
    API_ARRAY=($API_LENGTHS)

    # Compare sorted arrays
    if [ "${OAUTH_ARRAY[*]}" != "${API_ARRAY[*]}" ]; then
      echo "❌ ERROR: maxLength values differ between auth paths"
      echo "   OAuth path (server.ts): ${OAUTH_ARRAY[*]}"
      echo "   API key path (api-key-handler.ts): ${API_ARRAY[*]}"
      echo "   Fix: Use identical maxLength in both sanitizeOutput() calls"
      echo "   Example: { maxLength: 10000 } in both files"
      ERRORS=$((ERRORS + 1))
    else
      echo "✅ maxLength consistent across auth paths: ${OAUTH_ARRAY[*]}"
    fi
  fi
fi
echo ""
